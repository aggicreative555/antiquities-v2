...
function Checkout() {
  ...
  return (
    <div className="container h-screen mx-auto px-8">
      <div className='flex flex-row justify-between items-center relative w-full mb-10 mt-2'>
        <p className='text-sm text-amber-950 '>Back</p>
        <h1 className='absolute left-1/2 transform -translate-x-1/2 uppercase text-2xl text-amber-950'>Your cart</h1>
      </div>
      <div>
      ...
      <ul className="space-y-4">
        {cart.map((item) => ( 
          <li
          key={item.id}
          className="flex md:flex-row flex-col md:items-center gap-4 justify-between border p-4 rounded shadow">
            ...
            <div className='flex flex-col justify-start items-start h-full  gap-2 w-full'>
              <div className="flex flex-row justify-between w-full items-center">
                ...
                <button
                  ...
                  className="text-amber-950 text-bold text-2xl py-1 px-2 transition-all duration-150 ease-in-out hover:text-red-800 hover:bg-red-100"
                  onClick={() => removeFromCart(item.id)}> X
                </button>
              </div>
              <p className="text-sm text-gray-600">{item.price} NOK</p>
              <Increment item={item}/>
            </div>
          </div>
        </li>
      ...
      <div className="flex flex-col md:flex-row gap-6 h-fit my-4 md:items-center w-full">
        <p className='uppercase w-fit text-nowrap'>Price {total} NOK</p>

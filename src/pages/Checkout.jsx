  ...
  return (
    <div className="container h-fit mx-auto px-8">
      <div className="flex flex-row justify-between items-center relative w-full mb-10 mt-2">
        <Link to="" className="text-sm text-amber-950 -z-10">
          Back
        </Link>
        <h1 className="absolute left-1/2 transform -translate-x-1/2 uppercase text-2xl text-amber-950 -z-10">
          Your cart
        </h1>
      </div>
      <div>
        <p className="text-lg text-amber-950 my-4">Your products</p>
        <div>
                ...
                <div className="gap-6 flex flex-row">
                  ...
                  <div className="flex flex-col justify-start items-start h-full  gap-2 w-full">
                    <div className="flex flex-row justify-between w-full items-center">
                      <Link to={`/products/${product.id}`} key={product.id}>
                        <h2 className="text-2xl border-b-0 hover:border-b-2  hover:border-amber-950 transition-all duration-50">
                          {product.title}
                        </h2>
                      </Link>
                      <button
          ...
          <div className="flex flex-col md:flex-row gap-6 h-fit my-4 md:items-center w-full">
            <p className="uppercase w-fit text-nowrap">
              Price {Math.round(total)} NOK
            </p>
            <button
